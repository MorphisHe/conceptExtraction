...
class EmbedRank:
    ...
    def gen_freq_dict(self, doc_token):
        '''
        This method updates class attribute freq_dict with
        word frequencies inside doc_token
        
        Parameter:
        ---------------
        doc_token: a 2d list that contains all sentence in document as list.
                   sentence level list contains all candidate key phrases
                   extracted
        '''
        for sent_token in doc_token:
            for kp_token in sent_token:
                for word_token in kp_token.split():
                    self.freq_dict[word_token] += 1

    def freq_filter(self, doc_token, threshold=2):
        '''
        This method filter out all words that has frequency < threshold
        ...
        threshold: an int to indicate the min frequency of words to keep

        Return:
        ---------------
        filtered_doc: list with same dimension as doc_token with all low
                      frequency words removed
        '''
        filtered_doc = []
        for sent_id in range(len(doc_token)):
            sent_token = doc_token[sent_id]
            new_sent_token = []
            for ckp_token in sent_token:
                filtered_ckp = []
                for word_token in ckp_token.split():
                    if self.freq_dict[word_token] >= threshold:
                        filtered_ckp.append(word_token)
                if len(filtered_ckp):
                    new_sent_token.append(' '.join(filtered_ckp))
            if len(new_sent_token):
                filtered_doc.append(new_sent_token)
        return filtered_doc